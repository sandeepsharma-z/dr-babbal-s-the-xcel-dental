...
const botReply = (input: string) => {
  ...
  if (/(hi|hello|salam|assalam|hey)/.test(q)) {
    return "Hello! Main Dr Babbal's The XCEL dental assistant hoon. Aap implant, pain, braces, cleaning, timing ya booking ke baare me pooch sakte hain.";
  }
  ...
  if (/(pain|dard|toothache|sujaan|swelling)/.test(q)) {
    return "Agar severe pain ya swelling ho to delay mat karein. Garam-thanda avoid karein aur emergency checkup ke liye clinic visit karein ya call karein: 080765 97049.";
  }
  ...
  if (/(appointment|book|booking|schedule)/.test(q)) {
    return "Appointment ke liye page par Book Now form fill karein ya direct call karein: 080765 97049.";
  }
  ...
  if (/(emergency|urgent|bleeding|broken tooth)/.test(q)) {
    return "Emergency case me foran contact karein: 080765 97049. Hum urgent dental pain, fracture aur infection cases handle karte hain.";
  }
